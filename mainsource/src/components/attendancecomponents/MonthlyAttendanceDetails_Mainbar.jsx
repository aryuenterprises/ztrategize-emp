...
const MonthlyAttendanceDetails_Mainbar = () => {
  ...
  // console.log("employeeemail ID:", employeeeid);
  ...
  const columns = [
    ...
    //               : ""
    //             }`}
    //         >{`${rowData?.result?.breakTime?.hours}:${rowData?.result?.breakTime?.minutes}:${rowData?.result?.breakTime?.seconds}-(${rowData?.result?.totalBreakInCount}) `}</p>
    //       </div>
    //     ) : (
    //       "-"
    //     ),
          ...
          >
            ...
            >
              {`${rowData?.result?.breakTime?.hours ?? 0}:${rowData?.result?.breakTime?.minutes ?? 0
                }:${rowData?.result?.breakTime?.seconds ?? 0} (${rowData?.result?.totalBreakInCount ?? 0
                })`}
            </p>
          </div>
        ) : (
    ...
    ,
    {
      ...
      body: (rowData) =>
        rowData.result ? (
          <p>
            {`${String(rowData?.result?.totalWorkTime?.hours).padStart(
              2,
              "0"
            )}:${String(rowData?.result?.totalWorkTime?.minutes).padStart(
              2,
              "0"
            )}:${String(rowData?.result?.totalWorkTime?.seconds).padStart(
              2,
              "0"
            )}`}
          </p>
  ...
  return (
    <div className="flex flex-col justify-between overflow-x-hidden bg-gray-100 min-h-screen px-5 pt-2 md:pt-5 w-screen ">
      ...
      {tooltipData?.data?.length > 0 && (
        <div
          className="absolute bg-[#fafafa] border border-gray-300 shadow-lg px-5 py-5 rounded text-xs z-50 transition-opacity duration-200 max-w-[250px] max-h-[200px] overflow-y-auto"
          style={{
            top:
              tooltipData.y + 10 + 200 > window.innerHeight
                ? tooltipData.y - 150
        ...
        >
          ...
          {tooltipData.data
            ...
            .map((entry, index) => (
              <div key={index} className="mt-2 flex items-center justify-between gap-4">
                <h2 className="text-[16px] text-gray-800">{entry.reason}</h2>
                <p className="text-gray-700">
                  {new Date(entry.time)
                    .toLocaleTimeString("en-GB", { hour12: false })
                    .slice(0, 8)}
